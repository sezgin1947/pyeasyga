...
class GeneticAlgorithm(object):
    ...
    def run(self):
        ...
        for _ in range(1, self.generations):
            if _ % 5 == 0 and _ != 0:
                temp = copy.deepcopy(self.current_generation)
                self.create_first_generation()
                self.current_generation = temp[:self.population_size//2] + self.current_generation[self.population_size//2+1:]
            # if _ in [1,5,10,15,20,25,30]:
            # Below commented part is to save fitness data over time. Used for debugging.
            #     fitnesses_ = []
            #     genes = []
            #     for fitness_, gene in self.last_generation():
            #         fitnesses_.append(fitness_)
            #         genes.append(gene)
            #     self.fitnesses.append(fitnesses_)
            #     self.genes.append(genes)
            fitness, genes = self.best_individual()
            print(f"Generation: {_} in {self.generations}, fitness: {fitness}")
